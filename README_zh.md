...
- **Settings**：可自愈且跨标签同步的语言/主题/显示币种偏好、浏览器阻止偏好持久化时的明确反馈、带 schema
  校验的历史备份/恢复与重置
- **Help**：使用说明与补充信息
...
- Black-Scholes 价格与 Greeks 对极端现价/行权价比例使用稳定的对数价值度与有限尾部极限
- NPV 与 IRR 会忽略追加的精确零现金流尾部（包括接近 `-100%` 利率奇点时），但不会掩盖真实非零折现值的
  不可表示情况
- TVM RATE 会保留兼容的迭代根，并在整数期现金流可能存在多个数学上有效的利率时提示 10% 初始值与兜底
  搜索语义
...
- 当前页面分类的最后一条历史消失时，History 会无空状态闪烁地回到“全部”筛选
...
- 存储反馈会区分“偏好已在会话生效但未保存”和“货币、恢复、清空或导入未完成/可能仅部分完成”
